...
def anneal_zone_sa(
    ...
    """Runs Simulated Annealing to refine optimal slotting for a single zone."""
    ...
    # Sort zone_skus by safety tier and coi_score for initial Misfit/Baseline placement
    def get_tier(sku):
    ...
    # Initial State S0: Greedy Heuristic assignment as starting point
    curr_state = np.full(num_skus, -1, dtype=int)
    used_seed_slots = set()
    ...
    for i in range(num_skus):
        ...
        for j in range(num_slots):
            ...
            if chem_i:
                ...
                if not conflict:
                    for k in range(i):
                        if is_food[k] and curr_state[k] != -1:
                            sj = curr_state[k]
                            if (
                                s_pref == slot_pref[sj]
                ...
                if not conflict:
                    for k in range(i):
                        if is_chem[k] and curr_state[k] != -1:
                            sj = curr_state[k]
                            if (
                                s_pref == slot_pref[sj]
                                and ((s_x - slot_x[sj]) ** 2 + (s_z - slot_z[sj]) ** 2)
    ...
    def calc_energy(state):
        b_dist = slot_dist[state]
        b_level = slot_level[state]
        b_x = slot_x[state]
        b_z = slot_z[state]
        b_pref = slot_pref[state]
        ...
        return travel_cost + floor_penalty + (safety_violations * 500000.0)

    curr_energy = calc_energy(curr_state)
    ...
    for step in range(max_steps):
        for _ in range(iter_per_step):
            ...
            # Neighborhood operator: Pick 2 random SKUs and swap their assigned slots
            u, v = np.random.choice(num_skus, size=2, replace=False)
            neighbor = curr_state.copy()
            neighbor[u], neighbor[v] = neighbor[v], neighbor[u]

            neigh_energy = calc_energy(neighbor)
            delta_E = neigh_energy - curr_energy
    ...
    for s_idx, b_idx in zip(range(num_skus), best_state):
...
def run_sa_optimization(target_stock_codes=None, reoptimize_all=False):
    uri_pg = get_pg_connection_uri()

    mode_str = (
        "Complete SA Re-optimization"
        if reoptimize_all
        else "Targeted/Incremental SA Optimizer"
    )
    print(f"--- Starting {mode_str} (Simulated Annealing Local Search) ---")
    ...
    if not to_optimize.is_empty():
        ...
        for zone in zones:
            if zone is None:
                continue
            print(f"  Annealing SA population for zone: {zone}...")
            ...
            z_assigned = anneal_zone_sa(z_skus, z_slots, bh_z, bf_z)
            if z_assigned:
    ...
    if not final_layout.is_empty():
        ...
        final_layout.write_database(
            table_name="proposed_layout_sa",
            connection=uri_pg,
            if_table_exists="replace",
            engine="adbc",
        )
        print(
            f"SA Annealing complete. Refined optimal layout for {len(final_layout)} SKUs."
        )
    ...
    for arg in sys.argv[1:]:
        if arg == "--complete":
            ...
            try:
                ...
                pass
    run_sa_optimization(target_stock_codes=target_stocks, reoptimize_all=reoptimize_all)
